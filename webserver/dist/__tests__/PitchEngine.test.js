...
import { Suit, Phase, ACTION_PASS, ACTION_NO_VALID_PLAY, } from "../game/constants.js";
describe("PitchEngine", () => {
    ...
    describe("discard and fill", () => {
        ...
        it("off-jack is kept", () => {
            ...
            const offJack = { suit: Suit.DIAMONDS, rank: 12 };
    ...
    // --- Playing ---
    describe("playing", () => {
        function setupPlaying(e, trump = Suit.HEARTS) {
            e.step(11); // bid 5
            e.step(ACTION_PASS);
            e.step(ACTION_PASS);
    ...
    // --- Scoring ---
    describe("scoring", () => {
        function setupForScoring(e, bid, bidderTeam) {
            e.reset(0);
            e.currentBid = bid;
        ...
        it("bidding team goes set when bid is not made", () => {
            ...
            expect(roundEnd.data.totalScores[0]).toBe(3);
            // Team 1 keeps their 5: 10+5=15
            expect(roundEnd.data.totalScores[1]).toBe(15);
        });
        it("non-bidding team always banks their points", () => {
            ...
            expect(roundEnd.data.totalScores[1]).toBe(4);
        });
        it("moon (bid 11) awards 20 when all 10 points captured", () => {
            ...
            const roundEnd = events.find((ev) => ev.type === "roundEnd");
            expect(roundEnd.data.bidMade).toBe(true);
            expect(roundEnd.data.totalScores[0]).toBe(20);
        });
        it("moon (bid 11) penalizes 20 when failed", () => {
            ...
            const roundEnd = events.find((ev) => ev.type === "roundEnd");
            expect(roundEnd.data.bidMade).toBe(false);
            expect(roundEnd.data.totalScores[0]).toBe(0); // 20-20=0
        });
        it("double moon (bid 12) awards 40 when all 10 points captured", () => {
            ...
            const roundEnd = events.find((ev) => ev.type === "roundEnd");
            expect(roundEnd.data.bidMade).toBe(true);
            expect(roundEnd.data.totalScores[0]).toBe(40);
        });
        it("double moon (bid 12) penalizes 40 when failed", () => {
            ...
            const roundEnd = events.find((ev) => ev.type === "roundEnd");
            expect(roundEnd.data.bidMade).toBe(false);
            expect(roundEnd.data.totalScores[0]).toBe(10); // 50-40=10
        });
    });
    // --- Game end ---
    describe("game end", () => {
