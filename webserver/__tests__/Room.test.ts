...
describe("Room", () => {
  let room: Room;
  let onDestroy: ReturnType<typeof vi.fn>;
  ...
  describe("seat management", () => {
    ...
    it("AI players are marked as AI", () => {
      room.addAIPlayersAtSeats([0, 1, 2] as SeatIndex[]);
      for (let i = 0; i < 3; i++) {
    ...
    it("getSeats returns seat info for all players", () => {
      room.addPlayer("p1", "Alice", false);
      room.addAIPlayersAtSeats([1] as SeatIndex[]);
  ...
  describe("game lifecycle", () => {
    ...
    it("startGame sends game:start to human players", () => {
      ...
      expect(messages.some((m: any) => m.type === "game:start")).toBe(true);
    });
    ...
    it("handleAction validates it is the player's turn", () => {
      ...
      if (room.engine.currentPlayer !== 0) {
        ...
        const calls = (ws.send as ReturnType<typeof vi.fn>).mock.calls;
        const messages = calls.map((c) => JSON.parse(c[0] as string));
        expect(messages.some((m: any) => m.type === "error" && m.code === "NOT_YOUR_TURN")).toBe(true);
      }
    });
    ...
    it("handleAction validates game is in progress", () => {
      ...
      const calls = (ws.send as ReturnType<typeof vi.fn>).mock.calls;
      const messages = calls.map((c) => JSON.parse(c[0] as string));
      expect(messages.some((m: any) => m.type === "error" && m.code === "NOT_PLAYING")).toBe(true);
    });
  });
  ...
  describe("AI turns", () => {
    it("schedules timeout for AI player", () => {
      ...
      if (currentPlayer?.isAI) {
        ...
        expect((room as any)._aiTurnPending).toBe(true);
        expect((room as any)._aiTimeout).not.toBeNull();
      }
    });
    ...
    it("skips scheduling for human player", () => {
      ...
      room.state = "playing";
      (room as any)._aiTurnPending = false;
      (room as any)._aiTimeout = null;

      room.processAITurns();
      expect((room as any)._aiTurnPending).toBe(false);
      expect((room as any)._aiTimeout).toBeNull();
    });
    ...
    it("guard prevents concurrent AI turn scheduling (Bug 2 fix)", () => {
      ...
      room.processAITurns();
      expect((room as any)._aiTurnPending).toBe(true);
      const firstTimeout = (room as any)._aiTimeout;

      // Call again — should not overwrite
      room.processAITurns();
      expect((room as any)._aiTimeout).toBe(firstTimeout);
    });
  ...
  describe("broadcasting", () => {
    ...
    it("broadcast sends to all human players", () => {
      room.addPlayer("p1", "Alice", false);
      room.addPlayer("p2", "Bob", false);
      room.addAIPlayersAtSeats([2, 3] as SeatIndex[]);
      const ws1 = mockWs();
      const ws2 = mockWs();
      room.attachSocket("p1", ws1);
      room.attachSocket("p2", ws2);

      room.broadcast({ type: "pong" } as any);
